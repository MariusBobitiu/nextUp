    ...
    <>
        ...
        {images && (
            <div className='grid grid-cols-4 w-full'>
                {images?.backdrops.map((image: {file_path: string; id: string;}) => (
                    <img
                        key={image.id}
                        src={`https://image.tmdb.org/t/p/w500${image.file_path}`}
                        alt={movie.title}
                        className='w-96 h-64 m-4 rounded-lg object-cover shadow-lg'
                    />
                ))}
            </div>
        )}
...
export default MovieDetailsImages
